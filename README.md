...
Es un proyecto desarrollado en `React`. (v.18.0.0 - legacy render) y gestiona el **state** de la aplicación mediante `redux` (v.4.2.0). Debe servirse como **SPA** redirigiendo las peticiones a `index.html` para que el router de react `react-router-dom` (v.6.3.0) se haga cargo de la gestión de las **URLS**.
